...
namespace HrMatchApp.Forms
{
    public partial class WorkerSearchForm : Form
    {
        User activeWorker;
        CV cv;
        public WorkerSearchForm(User activeWorker)
        {
            InitializeComponent();
                ...
                for (int itemIndex = 0; itemIndex < listView.Items.Count; itemIndex++)
                {
                    ...
                    if (rectangle.Contains(e.Location) && subItems[0].Text == "APPLY HERE")
                    {
                        ...
                        if (dialogResult.Equals(DialogResult.Yes))
                        {
                            ...
                            ApplyAnnouncementForm applyAnnouncement = new ApplyAnnouncementForm(activeWorker, announcement);
                            applyAnnouncement.ShowDialog();
                        }
                    }
                }
            }
